...
def register_stat_calculators(
    ...
    if isinstance(model, BlackboxModel):
        ...
        _register(GreedyProbsCalculator(n_alternatives=n_ccp_alternatives, sample=True))
        _register(EntropyCalculator())
        _register(EntropyCalculator(sample=True))
        _register(GreedyLMProbsCalculator())
        _register(GreedyLMProbsCalculator(sample=True))
        ...
        _register(TokenCrossEncoderSimilarityMatrixCalculator(nli_model=nli_model))
        _register(TokenCrossEncoderSimilarityMatrixCalculator(nli_model=nli_model, sample=True))
